...
const WASM_URL = "/artifacts/semaphore30.wasm";
const ZKEY_URL = "/artifacts/semaphore30.zkey";
const VK_URL = "/artifacts/verification_key30.json";
...
async function fetchLeaves(
  ...
  for (let start = 0; start < leafCount; start += LEAF_FETCH_BATCH_SIZE) {
    ...
    onProgress?.(
      `Fetching commitments\u2026 ${Math.min(start + batchSize, leafCount)}/${leafCount}`,
    );
  }
  return leaves;
}
...
export async function generateProofClientSide({
  ...
  const optionsCount = toSafeNumber(
    readField(rawPoll, "options_count", 1) ?? 0,
    "Options",
  );
  if (option < 0 || option >= optionsCount) {
    throw new Error(
      `Option out of range for poll ${pollIdForCall}. Allowed options are 0\u2013${Math.max(optionsCount - 1, 0)}.`,
    );
  }

  const snapshotRoot = toBigIntValue(readField(rawPoll, "snapshot_root", 4) ?? 0);

  onProgress?.("Checking registry\u2026");
  ...
  if (leafCount > MAX_UI_LEAF_COUNT) {
    throw new Error(
      `This poll has ${leafCount} commitments. Client proof generation supports up to ${MAX_UI_LEAF_COUNT}.`,
    );
  }
  ...
  const leaves = await fetchLeaves(registryRead, pollIdForCall, leafCount, onProgress);
  const leafIndex = leaves.findIndex((leaf) => leaf === commitment);
  if (leafIndex < 0) {
    throw new Error(
      "Commitment from identity was not found in this poll. Register it before generating proof.",
    );
  }

  onProgress?.("Building Merkle proof\u2026");
  ...
  if (merkleProofLength > TREE_DEPTH) {
    throw new Error(
      `Merkle proof depth ${merkleProofLength} exceeds supported depth ${TREE_DEPTH}.`,
    );
  }
  ...
  onProgress?.("Converting proof to calldata\u2026");
  const vkResponse = await fetch(VK_URL);
  if (!vkResponse.ok) {
    throw new Error("Failed to fetch verification key.");
  }
  const vkData = (await vkResponse.json()) as RawVerificationKey;
